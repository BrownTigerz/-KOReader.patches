...
local B_SETT = {	--BANNER SETTINGS
					...
					margin = 10,
					padding = 15,	
					max_height = 40,		-- percentage of screen height
...
--picks a random index in [1, count], avoiding anything in recent_list's
--current window. Mutates recent_list to record the pick and trims it to
--the window size. Falls back to a smaller effective window if the pool
--is too small to avoid one (so it can never loop forever).
local function pickRandomIndex(count, recent_list, window)
	if count <= 1 then
		return 1
	end
	local eff_window = math.min(window, count - 1)
	local idx
	repeat
		idx = math.random(count)
	until not tableContains(recent_list, idx)
	table.insert(recent_list, idx)
	while #recent_list > eff_window do
		table.remove(recent_list, 1)
	end
	return idx
end
...
local function parseFooterText(text, index)
	...
	hl_date = yr and mth and dy and short_month and 
				string.format("%s %d '%02d", short_month, tonumber(dy), tonumber(yr) % 100) or "N/A"
...
local function loadQuotesFromFile(path)
	local mtime = lfs.attributes(path, "modification")

	--already cached and file hasn't changed since (or mtime is unreadable
	--but we have a cache from before) -- reuse it.
	if cached_quotes_list and (not mtime or mtime == cached_quotes_mtime) then
		return cached_quotes_list
	end

	local new_list = {}
	local f = io.open(path, "r")
	if not f then
		return cached_quotes_list or new_list
	end
	for line in f:lines() do
		local trimmed = util.trim(line)
		if trimmed ~= "" then
			table.insert(new_list, trimmed)
		end
	end
	f:close()

	cached_quotes_list = new_list
	cached_quotes_mtime = mtime
	return cached_quotes_list
end
...
function UIManager:show(widget, ...)
	...
	--fallback: no eligible highlight in this book, try a random quote from file
	local quoteCount, quoteEnabled, quotes_list
	
	if not highlightEnabled and QUOTE_SETT.enabled then
		quotes_list = loadQuotesFromFile(QUOTE_SETT.file_path)
		quoteCount = #quotes_list
		quoteEnabled = quoteCount > 0
	end
	...
	local max_wid
	if not highlightEnabled and not quoteEnabled then
	...
	elseif quoteEnabled then
		--fallback: random quote from txt file. no footer, no re-wrapping in
		--quote marks since lines in the file are expected to already be
		--formatted the way you want them shown (e.g. “Blah blah blah” - Quoter)
		local random_quote, random_quote_index
		
		random_quote_index = pickRandomIndex(
								quoteCount, 
								recent_quote_indices, 
								QUOTE_SETT.no_repeat_window
		)
	...
	if (highlightEnabled or quoteEnabled) and highlight_widget then
